...
class Cell:
    ...
    def __str__(self) -> str:
        return f"{f'(x={self.coord[0]:3}, y={self.coord[1]:3}) dist= {self.dist:5} p= {self.points}'}"
...
def solve(graph, max_x, max_y):
    ...
    while to_visit != []:
        cell = to_visit.pop()
        print(f"Cell {cell} | {time.time()-start:8.4f}s", end="\r")
...
def graph_maker(a, extend=1):
    ...
    for j in range(org_size[0]):
        for i in range(extend):
            extended_grid[j+i*org_size[0]] = extended_rows[j][i *
                                                              org_size[1]:i*org_size[1]+org_size[1]*extend]
    ...
    for r in range(max_y):
        for c in range(max_x):
            graph[(c, r)] = Cell((c, r), extended_grid[r][c],
                                 get_adjacent_cells(c, r, neighbours, max_x, max_y))
